...
import {getStore} from '../../Function/Async';
...
const Records = (props) => {
  ...
  function VsTime({business}) {
    ...
    var today =
      date.getFullYear() + '-' + (date.getMonth() + 1) + '-' + date_getDate;
    if (
      today == data[business.index].visit_date.slice(0, 10) &&
      getTime.slice(0, 2) ==
        data[business.index].visit_date.slice(-8, -3).slice(0, 2)
    ) {
      if (
        getTime == data[business.index].visit_date.slice(-8, -3) ||
        parseInt(getTime.slice(3, 5)) ==
          parseInt(data[business.index].visit_date.slice(-8, -3).slice(3, 5)) +
            1
      ) {
        return true;
      } else {
